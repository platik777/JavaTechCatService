...
import ru.platik777.repositories.CatRepository;
import ru.platik777.repositories.OwnerRepository;
...
public class CatService {
    ...
    @Transactional
    public Cat createCatWithOwner(Cat cat, OwnerDto ownerDetails) {
        CatEntity catEntity = new CatEntity(cat.getName(), cat.getBirthday(),
                cat.getBreed(), cat.getAge(),cat.getColor());
        OwnerEntity ownerEntity = ownerRepository.findById(ownerDetails.getId()).orElse(null);
        catEntity.setOwnerEntity(ownerEntity);
        return new Cat(catRepository.save(catEntity));
    }
